...
function createOne(req, res, next) {
  ...
  Category.find({ category_name: req.body.category_name }).lean().exec().then((name) => {
    if (name.length > 0) {
...
function getAll(req, res, next) {
  Category.find().select('_id category_name').lean().exec().then((data) => {
...
function getOne(req, res, next) {
  Category.findOne({ _id: req.params.categoryId }).select('_id category_name').lean().exec().then((data) => {
    res.json({ success: true, data })
  })
    .catch(e => {
      const err = new APIError(e.message, httpStatus.METHOD_NOT_ALLOWED, true);
